			...
			<?foreach($arResult as $i => $arItem):?>
				...
				<?$bChilds = $arItem["CHILD"] || ( $i == 0 && isset($arResult[1]) );?>
 				<div class="item <?=$i?> <?=$bChilds ? 'childs' : ''?> <?=($arItem["SELECTED"] ? " active" : "")?> <?=($bAccordion ? " accordion-close" : "")?> " <?=($bAccordion ? 'data-parent="#'.$indexSection.'" data-target="#'.$indexSection.'"' : '')?> >
					<div class="title">
						<?if($arItem['PARAMS']['FEEDBACK'] == 'Y'):?>
						<a><span data-event="jqm" data-param-form_id="ASK" data-name="contacts"><?=$arItem['TEXT']?></span></a>
						<?elseif($arItem['PARAMS']['REVIEW'] == 'Y'):?>
						<a><span data-event="jqm" data-param-form_id="REVIEW" data-name="send_review"><?=$arItem['TEXT']?></span></a>
						<?elseif($bLink):?>
